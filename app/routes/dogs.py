...
from app.classes.data import Dogs, Comment
from app.classes.forms import DogsForm, CommentForm
...
def dogsNew():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('dogs',dogsID=newDogs.id))

    # if form.validate_on_submit() is false then the user either has not yet filled out
    # the form or the form had an error and the user is sent to a blank form. Form errors are 
    # stored in the form object and are displayed on the form. take a look at blogform.html to 
    # see how that works.
    return render_template('dogsform.html',form=form)


@app.route('/dogs<dogsID>')
# This route will only run if the user is logged in.
@login_required
def dogs(dogsID):
    thisDogs = Dogs.objects.get(id=dogsID)
    ...
    theseComments = Comment.objects(dogs=thisDogs)
    # Send the blog object and the comments object to the 'blog.html' template.
    return render_template('dogs.html',dogs=thisDogs,comments=theseComments)